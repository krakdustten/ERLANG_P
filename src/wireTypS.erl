...
init() -> 
	survivor:entry(wireTypS_created),
	loop().
...
loop() -> 
	receive
		{initial_state, [ResInst_Pid, TypeOptions], ReplyFn} ->
			...
			C1 = connector:create(ResInst_Pid, simpleWire),
			C2 = connector:create(ResInst_Pid, simpleWire),
			ReplyFn(#{resInst => ResInst_Pid, chambers => [Location], 
					cList => [C1, C2], typeOptions => TypeOptions,
